...
export const SearchResultSchema = Type.Object({
  ...
  complexity: Type.String({ description: 'Complexity level: simple, intermediate, complex' }),
  language: Type.String({ description: 'Programming language: typescript, python' }),
  feature_tags: Type.Array(Type.String(), { description: 'Feature tags for categorization' }),
  features_to_demonstrate: Type.Array(Type.String(), { description: 'Features demonstrated' }),
  target_users: Type.Array(Type.String(), { description: 'Target user types' }),
  folder_name: Type.Optional(Type.String({ description: 'Folder name in repository' })),
...
export const SearchResponseSchema = Type.Object({
  results: Type.Array(SearchResultSchema, { description: 'Array of matching examples' }),
  query: Type.String({ description: 'The search query that was executed' }),
  count: Type.Integer({ minimum: 0, description: 'Number of results returned' }),
  processingTimeMs: Type.Number({ minimum: 0, description: 'Processing time in milliseconds' })
})
...
export const AlgoKitExampleSchema = Type.Object({
  ...
  vector: Type.Array(Type.Number(), { description: '384-dimensional embedding vector' })
})
...
export const HealthResponseSchema = Type.Object({
  ...
  examplesCount: Type.Integer({ minimum: 0, description: 'Number of examples in database' })
})
